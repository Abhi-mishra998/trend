...
The entire AWS environment — VPC, subnets, security groups, EKS cluster, node groups, and Jenkins EC2 — is defined and reproducible in Terraform. Spin up or tear down the whole stack with one command.

**4. Horizontal Pod Autoscaling**
HPA automatically scales pods from 3 to 10 replicas when CPU utilization exceeds 70%, then scales back down — optimizing both performance and cost without manual intervention.
...
|--------|-----------------|-------------------|
| **Deployment Time** | ~30 minutes | ~4 minutes |
| **Downtime per Deploy** | Variable / service restart | Zero (rolling update) |
| **Pod Scaling** | Fixed 3 pods | Auto-scales 3 → 10 pods |
| **Infrastructure Provisioning** | Manual clicks in AWS Console | ~20 min via `terraform apply` |
...
6. **Monitoring**: Prometheus and Grafana for full observability
